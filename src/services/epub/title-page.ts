...
export function titlePage(book: Book): string {
  const m = book.metadata;
  const labels = getLabels(m.language);
  const lines = [`<h1>${esc(m.title)}</h1>`];
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE html>\n<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" xml:lang="${lang}" lang="${lang}"><head><meta charset="UTF-8"/><title>${esc(m.title)}</title><link rel="stylesheet" type="text/css" href="theme.css"/><link rel="stylesheet" type="text/css" href="custom.css"/></head><body><section epub:type="titlepage">${lines.join("")}</section></body></html>`;
}
